...
map f xs = foldr (\x a -> f x :. a) Nil xs
...
filter f xs = 
  foldr (\x a -> if f x then x :. a else a) Nil xs 
...
-}
toListH :: List a -> [a]
toListH xs = foldr (\x a -> x : a) [] xs

-- И обратно
fromListH :: [a] -> List a
fromListH xs = P.foldr (\x a -> x :. a) Nil xs
...
intercalate :: [a] -> [[a]] -> [a]
intercalate sep ls = 
  P.foldr (\x a -> if null a then x else x ++ sep ++ a) [] ls
